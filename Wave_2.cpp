...
int main(int argc, char* argv[]) {
    ...
    SDL_Surface* shipSurface = IMG_Load("C:/UsersTran Dai Nghia/Desktop/Space Legend/Image/Spaceship/Ship3.png");
    if (!shipSurface) {
        std::cerr << "Failed to load square image: " << IMG_GetError() << std::endl;
        return 1;
    }
    ...
    const int square = 18;    float coorX=30,coorY=330;   int i=0;
    ...
    float bulletX[18],bulletY[18];     i=0;
    int bulletW[18],bulletH[18];
    while(i<square)
        {
    ...
    while (!quit) {
        ...
        for (int j = 0; j < boss; j++){SDL_RenderCopy(renderer, shipbossTexture, NULL, &boss2[j]);}
